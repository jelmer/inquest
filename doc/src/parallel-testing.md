...
If both test listing and filtering (via either `IDLIST` or `IDFILE`) are configured then inq is able to run your tests in parallel
...
is simple round-robin for tests that inq has not seen run before, and
equal-time buckets for tests that inq has seen run. NB: This uses the anydbm
...
To determine how many CPUs are present in the machine, inquest will
...
When running tests in parallel, inquest tags each test with a tag for
...
```text
  ======================================================================
  label: inquest.tests.ui.TestDemo.test_methodname
  tags: foo worker-0
  ----------------------------------------------------------------------
  error text
```
...
```sh
  $ inq last --subunit | subunit-filter -s --xfail --with-tag=worker-3 | subunit-ls > slave-3.list
```
